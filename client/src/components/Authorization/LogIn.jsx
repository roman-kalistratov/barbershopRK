...
const LogIn = () => {
    ...
    return (
        <Sidebar title="התחברות">
            <div className="authorization">
                <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" placeholder="כתובת אימייל" />
                <div className="authorization__input-password">
                    ...
                    <input value={password} id="password" onChange={(e) => setPassword(e.target.value)} type={`${isView ? "text" : "password"}`} placeholder="סיסמה" />
                </div>
                {error && <div className="authorization__message error">{error}</div>}
                <Spinner />
                <div className="authorization__btns">
                    <button className="authorization__btn btn" onClick={() => dispatch(login(email, password))}>התחברות</button>
                    <button className="authorization__btn btn" onClick={() => dispatch(showSidebar('registration'))}>הרשמה</button>
                </div>

                {!isAutoFill && <p className='authorization__autoFill-text' onClick={() => setIsAutoFill(!isAutoFill)}>לחץ כאן כדי למלא נתונים אוטומטית לבדיקה.</p>}
