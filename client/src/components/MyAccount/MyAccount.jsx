...
const MyAccount = () => {
  ...
  const hundleCancel = async (crntDate) => {
    try {
      dispatch(showLoader())
  ...
  return (
    <Sidebar title="החשבון שלי" setMessage = {setMessage}>
      <div className="myAccount">
        <div className="myAccount__logout" onClick={() => handleLogout()}>להתנתק <HiLogout /></div>
        <div className="myAccount__appointments">
          <div className="myAccount__appointments-header">
            <TbCalendarStats /> <span>המועדים שלי:</span>
          </div>
          <DatePicker
            key="example9"
            selected={startDate}
...

      {
        appointments && appointments.map(date => (
          ...
          <div className='myAccount__appointments-description'>
            יש לי תור ב:
            <span>{format(new Date(date.dateTime), "H:mm")}</span>
            <button className='btn' onClick={() => hundleCancel(date.dateTime)}>לבטל</button>
          </div>
        ))
      }
      {
        unavailableDates && unavailableDates.map(date => (
          ...
          <div className='myAccount__appointments-description'>
            ...
            <button className='btn' onClick={() => hundleCancel(date)}>לבטל</button>
          </div>
        ))
      }
