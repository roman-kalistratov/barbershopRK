...
const Booking = () => {
    ...
    const [title, setTitle] = useState("קביעת מועד");
    ...
    return (
        <Sidebar title={title} setIsDisabled = {setIsDisabled} setMessage = {setMessage}>
            <div className="booking">
                {isActiveCategory && <div className="booking__back" onClick={handleBack}>חזרה<IoIosArrowBack /></div>}
...

                {
                    ...
                    appointment.service &&
                    !message &&
                    <button className='booking__btn btn active' onClick={handleSubmit}>להזמין</button>
                }
            </div>
        </Sidebar>
    )
